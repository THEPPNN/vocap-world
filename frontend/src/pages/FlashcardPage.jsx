...
export default function FlashcardPage() {
  ...
  const handleMaster = async () => {
    ...
    try {
      ...
      setVocabs(vs => vs.map(v => v.id === card.id ? { ...v, is_mastered: true } : v));
      // Remove from deck
      ...
      <div className="page-wrapper flashcard-page">
        <div className="empty-state">
          ...
          <div style={{ display: 'flex', gap: '.75rem', justifyContent: 'center', flexWrap: 'wrap', marginTop: '1.25rem' }}>
            <button className="btn btn-primary" onClick={restartAll}>Practice all again</button>
            <Link to="/mastered" className="btn btn-secondary">View mastered words</Link>
          </div>
        </div>
      </div>
    );
  ...
  if (done) {
    ...
    return (
      <div className="page-wrapper flashcard-page">
        <h1 className="page-title" style={{ marginBottom: '1.5rem' }}>Session Complete 🎉</h1>
        <div className="card fc-result-card">
          <div className="fc-result-score">{pct}%</div>
          <div className="fc-result-label">Remembered {known} of {deck.length} words</div>
          {masteredCount > 0 && (
            <p style={{ color: 'var(--success)', fontSize: '.9rem', marginBottom: '1rem' }}>
              🏆 {masteredCount} word{masteredCount !== 1 ? 's' : ''} mastered total
            </p>
          )}
          <div style={{ display: 'flex', gap: '.75rem', justifyContent: 'center', flexWrap: 'wrap' }}>
            <button className="btn btn-primary btn-lg" onClick={restart}>🔄 Play again</button>
            {masteredCount > 0 && (
              <Link to="/mastered" className="btn btn-secondary btn-lg">🏆 View mastered</Link>
            )}
    ...
    <div className="fc-page-layout">
      ...
      <div className="flashcard-scene" onClick={() => setFlipped(f => !f)}>
        <div className={`flashcard-inner ${flipped ? 'flipped' : ''}`}>
          ...
          <div className="flashcard-face flashcard-back">
            ...
            <div className="fc-hint" style={{ color: 'var(--text-3)', marginTop: '.75rem' }}>
              Tap to flip back
            </div>
          </div>
        </div>
      ...
      <div className="fc-controls-wrap">
        <div className="flashcard-controls">
          <button className="btn fc-unknown-btn btn-lg" onClick={() => advance(false)}>
            ❌ Still learning
          </button>
          <span className="fc-counter">{known} known</span>
          <button className="btn fc-known-btn btn-lg" onClick={() => advance(true)}>
            ✅ Got it!
          </button>
        </div>
        ...
        <Link to="/" className="fc-back-link">← Back to library</Link>
      </div>
    </div>
  );
}